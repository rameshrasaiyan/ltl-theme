<?php
/**
 * Custom Header functionality for ranker
 *
 * @package WordPress
 * @subpackage ranker
 * @since ranker 1.0
...
function ranker_custom_header_setup() {
	...
	add_theme_support( 'custom-header', apply_filters( 'ranker_custom_header_args', array(
		...
		'width'                  => 954,
		'height'                 => 1300,
		'wp-head-callback'       => 'ranker_header_style',
	) ) );
}
...
if ( ! function_exists( 'ranker_header_style' ) ) :
/**
 * Styles the header image and text displayed on the blog.
 *
 * @since ranker 1.0
 *
 * @see ranker_custom_header_setup()
 */
function ranker_header_style() {
	...
	?>
	<style type="text/css" id="ranker-header-css">
	<?php
 ...
 */
function ranker_header_background_color_css() {
	$color_scheme            = ranker_get_color_scheme();
	...
	wp_add_inline_style( 'ranker-style', sprintf( $css, $header_background_color ) );
}
add_action( 'wp_enqueue_scripts', 'ranker_header_background_color_css', 11 );
 ...
 * @since ranker 1.0
 */
function ranker_sidebar_text_color_css() {
	$color_scheme       = ranker_get_color_scheme();
	...
	// If we get this far, we have custom styles. Let's do this.
	$sidebar_link_color_rgb     = ranker_hex2rgb( $sidebar_link_color );
	...
	wp_add_inline_style( 'ranker-style', sprintf( $css, $sidebar_link_color, $sidebar_text_color, $sidebar_border_color, $sidebar_border_focus_color ) );
}
add_action( 'wp_enqueue_scripts', 'ranker_sidebar_text_color_css', 11 );
