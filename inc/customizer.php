<?php
/**
 * ranker Customizer functionality
 *
 * @package WordPress
 * @subpackage ranker
 * @since ranker 1.0
...
function ranker_customize_register( $wp_customize ) {
	...
	$wp_customize->add_setting( 'color_scheme', array(
		'default'           => 'default',
		'sanitize_callback' => 'ranker_sanitize_color_scheme',
		'transport'         => 'postMessage',
	) );

	$wp_customize->add_control( 'color_scheme', array(
		'label'    => __( 'Base Color Scheme', 'ranker' ),
		'section'  => 'colors',
		'type'     => 'select',
		'choices'  => ranker_get_color_scheme_choices(),
		'priority' => 1,
	) );
	...
	$wp_customize->add_control( new WP_Customize_Color_Control( $wp_customize, 'sidebar_textcolor', array(
		'label'       => __( 'Header and Sidebar Text Color', 'ranker' ),
		'description' => __( 'Applied to the header on small screens and the sidebar on wide screens.', 'ranker' ),
		'section'     => 'colors',
	) ) );
	...
	$wp_customize->add_control( new WP_Customize_Color_Control( $wp_customize, 'header_background_color', array(
		'label'       => __( 'Header and Sidebar Background Color', 'ranker' ),
		'description' => __( 'Applied to the header on small screens and the sidebar on wide screens.', 'ranker' ),
		'section'     => 'colors',
	) ) );

	// Add an additional description to the header image section.
	$wp_customize->get_section( 'header_image' )->description = __( 'Applied to the header on small screens and the sidebar on wide screens.', 'ranker' );
}
add_action( 'customize_register', 'ranker_customize_register', 11 );

/**
 * Render the site title for the selective refresh partial.
 *
 * @since ranker 1.5
 * @see ranker_customize_register()
 *
 * @return void
 */
...
/**
 * Register color schemes for ranker.
 *
 * Can be filtered with {@see 'ranker_color_schemes'}.
 *
 ...
 */
function ranker_get_color_schemes() {
	/**
	 * Filter the color schemes registered for use with ranker.
	 *
	 ...
	 * }
	 */
	return apply_filters( 'ranker_color_schemes', array(
		'default' => array(
			'label'  => __( 'Default', 'ranker' ),
			'colors' => array(
				'#f1f1f1',
				'#ffffff',
				'#ffffff',
...
if ( ! function_exists( 'ranker_get_color_scheme' ) ) :
/**
 * Get the current ranker color scheme.
 *
 * @since ranker 1.0
 *
 * @return array An associative array of either the current or default color scheme hex values.
 */
function ranker_get_color_scheme() {
	$color_scheme_option = get_theme_mod( 'color_scheme', 'default' );
	$color_schemes       = ranker_get_color_schemes();
...
endif; // ranker_get_color_scheme

if ( ! function_exists( 'ranker_get_color_scheme_choices' ) ) :
/**
 * Returns an array of color scheme choices registered for ranker.
 *
 * @since ranker 1.0
 *
 * @return array Array of color schemes.
 */
function ranker_get_color_scheme_choices() {
	$color_schemes                = ranker_get_color_schemes();
...
endif; // ranker_get_color_scheme_choices

if ( ! function_exists( 'ranker_sanitize_color_scheme' ) ) :
/**
 ...
 */
function ranker_sanitize_color_scheme( $value ) {
	$color_schemes = ranker_get_color_scheme_choices();
...
endif; // ranker_sanitize_color_scheme
 ...
 */
function ranker_color_scheme_css() {
	$color_scheme_option = get_theme_mod( 'color_scheme', 'default' );
	...
	$color_textcolor_rgb         = ranker_hex2rgb( $color_scheme[3] );
	$color_sidebar_textcolor_rgb = ranker_hex2rgb( $color_scheme[4] );
	$colors = array(
	...
	$color_scheme_css = ranker_get_color_scheme_css( $colors );

	wp_add_inline_style( 'ranker-style', $color_scheme_css );
}
add_action( 'wp_enqueue_scripts', 'ranker_color_scheme_css' );
 ...
 * @since ranker 1.0
 */
function ranker_customize_control_js() {
	...
	wp_localize_script( 'color-scheme-control', 'colorScheme', ranker_get_color_schemes() );
}
add_action( 'customize_controls_enqueue_scripts', 'ranker_customize_control_js' );
...
function ranker_customize_preview_js() {
	wp_enqueue_script( 'ranker-customize-preview', get_template_directory_uri() . '/js/customize-preview.js', array( 'customize-preview' ), '20141216', true );
}
 ...
 */
function ranker_get_color_scheme_css( $colors ) {
 ...
 * @since ranker 1.0
 */
function ranker_color_scheme_css_template() {
	$colors = array(
	...
	?>
	<script type="text/html" id="tmpl-ranker-color-scheme">
		<?php echo ranker_get_color_scheme_css( $colors ); ?>
	</script>
	<?php
}
add_action( 'customize_controls_print_footer_scripts', 'ranker_color_scheme_css_template' );
